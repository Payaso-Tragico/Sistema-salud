...
public class PanelPacientes extends JPanel {
    ...
    public PanelPacientes(Hospital hospital, VentanaPrincipal ventanaPrincipal) {
        ...
        String[] columnas = {"RUT", "Nombre", "Edad", "Diagnóstico"};
    ...
    private void cargarPacientes() {
        ...
        for (Paciente p : hospital.getPacientes()) {
            modeloTabla.addRow(new Object[]{p.getRut(), p.getNombre(), p.getEdad(), p.getDiagnostico()});
        }
    }

    private void filtrarTabla(String texto) {
        List<Paciente> filtrados = hospital.getPacientes().stream()
                .filter(p -> p.getRut().contains(texto) || p.getNombre().toLowerCase().contains(texto.toLowerCase()))
