...
public PanelMedicos(Hospital hospital, VentanaPrincipal ventanaPrincipal) {
    ...
    private void cargarMedicos() {
        ...
        for (Medico m : hospital.getMedicos()) {
            modeloTabla.addRow(new Object[]{m.getRut(), m.getNombre()});
        }
    }

    private void filtrarTabla(String texto) {
        List<Medico> filtrados = hospital.getMedicos().stream()
                .filter(m -> m.getRut().toLowerCase().contains(texto.toLowerCase()) ||
                             m.getNombre().toLowerCase().contains(texto.toLowerCase()))
